#!/usr/bin/env bash
# Deploy a Railway service using a service-specific railway.json (temporary root swap).
#
# Usage:
#   bash scripts/railway-deploy-from-manifest.sh <service> <environment> <manifest-dir>
# Example:
#   bash scripts/railway-deploy-from-manifest.sh kafka production deploy/kafka
set -euo pipefail
