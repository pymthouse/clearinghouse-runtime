...
// TenantRecord holds tenant metadata only. It deliberately stores no long-lived
// plaintext credentials: SPAT values are returned to the caller once at creation
// and never persisted. Only the opaque IngestTokenID is kept so the current token
