...
type Config struct {
	...
	OpenMeterURL            string
	OpenMeterProvisionerPAT string
...
func LoadFromEnv() (Config, error) {
	cfg := Config{
		...
		InternalListenAddr:      envDefault("TENANT_ADMIN_INTERNAL_LISTEN_ADDR", "127.0.0.1:8094"),
	...
	// Optional defense-in-depth for the loopback/private internal API.
	cfg.InternalAPISecret = strings.TrimSpace(os.Getenv("INTERNAL_API_SECRET"))
	cfg.KonnectPlatformToken = strings.TrimSpace(os.Getenv("KONNECT_PLATFORM_PAT"))
	// The provisioner credential is the long-lived "clearinghouse-provisioner" SPAT used
	// for all OpenMeter catalog/customer/subscription operations. It is held by this
	// service and never returned to integrators.
	cfg.OpenMeterProvisionerPAT = strings.TrimSpace(os.Getenv("OPENMETER_PROVISIONER_PAT"))
	...
	if cfg.OpenMeterProvisionerPAT == "" {
		return Config{}, fmt.Errorf("OPENMETER_PROVISIONER_PAT is required")
	}
	if cfg.Auth0Domain == "" {
