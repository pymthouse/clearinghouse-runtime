    ...
    try {
      ...
      if (Array.isArray(parsed)) {
        for (const row of parsed) {
          ...
          if (auth0ClientId && tenantId) {
            map.set(auth0ClientId, tenantId);
          }
        }
      }
    } catch {
 ...
 * Optional DEMO_TENANT_ID or tenant registry resolves tenantId.
...
export function loadApiKeyStore(env) {
  ...
  const registry = createTenantRegistryIndex(env);

  const primaryKey = env.DEMO_API_KEY?.trim();
  if (primaryKey) {
    const clientId = env.DEMO_CLIENT_ID?.trim() || "demo-client";
    store.set(primaryKey, {
      tenantId:
        env.DEMO_TENANT_ID?.trim() ||
        clientTenantMap.get(clientId) ||
        registry.resolveTenantId(clientId) ||
        "default",
      clientId,
  ...
  if (extra) {
    ...
    if (parsed && typeof parsed === "object") {
      for (const [apiKey, entry] of Object.entries(parsed)) {
        ...
        const clientId = entry.clientId?.trim() || entry.client_id?.trim() || "demo-client";
