...
// Provisioner idempotently ensures Konnect customers and plan subscriptions.
type Provisioner struct {
	sdk     *sdkkonnectgo.SDK
	planKey string
}

// ProvisionInput identifies the tenant end user to provision.
...
// ProvisionResult is returned after ensure completes.
...
// BuildCustomerKey forms the OpenMeter customer key from client and user ids.
...
// NormalizeOpenMeterURL strips ingest suffixes for the Konnect SDK base URL.
...
// NewProvisioner constructs a provisioner backed by Kong sdk-konnect-go.
...
// Ensure creates customer and subscription when missing.
...
// FindCustomerByKey resolves a Konnect customer ID from the customer key, or "" if not found.
func (p *Provisioner) FindCustomerByKey(ctx context.Context, customerKey string) (string, error) {
	return p.findCustomerByKey(ctx, customerKey)
}
