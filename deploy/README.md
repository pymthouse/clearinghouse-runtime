...
```bash
cp .env.example .env
$EDITOR .env  # fill WEBHOOK_SECRET, OPENMETER_API_KEY at minimum
...
```bash
docker compose -f deploy/docker-compose.yml --env-file .env port remote-signer 4935
# expected: no output / error (port is not mapped)
docker compose -f deploy/docker-compose.yml --env-file .env port remote-signer 8081
...
| `OPENMETER_DEFAULT_PLAN_KEY` | no | `clearinghouse_default_ppu` | Default pay-per-use plan key (Konnect) |
...
Konnect API calls use the workspace package `@pymthouse/konnect-metering` (`packages/konnect-metering`). Its typed client is generated from Kong's hosted OpenAPI spec and committed as `packages/konnect-metering/src/schema.gen.ts`. Refresh after API changes:

```bash
pnpm --filter @pymthouse/konnect-metering generate
```

Source: [Konnect Metering & Billing v3 OpenAPI](https://raw.githubusercontent.com/Kong/developer.konghq.com/main/api-specs/konnect/metering-and-billing/v3/openapi.yaml)
...
| Plan (new) | `clearinghouse_default_ppu` | Pay-per-use: $0.000001/billable micro + **$5 included usage** |
...
| `billable_usd_micros` / `billable_spend` / `clearinghouse_default_ppu` | — | new |
